...
func (cli *TelegramCLI) parseMessage(message mtproto.TL) {
	...
	case mtproto.TL_message:
		...
		case mtproto.TL_peerChat:
			...
			message := fmt.Sprintf(
				"%s %d %s in %s(%d): %s",
				date, message.Id, senderName, chat.Title, chat.Id, message.Message)
			fmt.Println(message)
		case mtproto.TL_peerChannel:
			...
			message := fmt.Sprintf(
				"%s %d %s in %s(%d): %s",
				date, message.Id, senderName, channel.Title, channel.Id, message.Message)
			fmt.Println(message)
		default:
...
func (cli *TelegramCLI) RunCommand(command *Command) error {
	...
	case "umsg":
		...
		update, err := cli.mtproto.MessagesSendMessage(
			...
			0, args[1], rand.Int63(), mtproto.TL_null{}, nil)
